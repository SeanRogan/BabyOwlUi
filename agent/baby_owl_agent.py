...
"""THIS AGENT IS THE MAIN WORKER OF THE PROGRAM. IT CREATES A TASK LIST BASED ON THE INITIAL USER PROMPT,
 IN AN ATTEMPT TO RESEARCH THE PROBLEM AND RETURN RELEVANT INFORMATION."""
OBJECTIVE = None
task_list = []
# task_list = deq
...
def execute_task(task: Dict, task_list: List, obj: str):
    OBJECTIVE = obj
    global token_count
    task_output = None
    ...
    if task["tool"] == "text-completion":
        ...
        task_output = web_search_tool(task_query)
...
def fly():
    ...
    task_list = create_task_list(OBJECTIVE)
    print_task_list()
